...
(defn with-animation [value & [options callback]]
         ...
         (when (and (not disable-drag?) show-bottom-sheet?)
           (let [max-pan-up (if (or @expanded? (not expandable?))
                 ...
                 max-pan-down (if @expanded?
                                bg-height-expanded
                                bg-height)]
             (reanimated/set-shared-value pan-y (max
                                                 (min
                                                  (.-translationY evt)
                                                  max-pan-down)
                                                 max-pan-up))))))
       ...
       [:f>
        (fn []
          (let [{window-height :height
                ...
                bg-opacity (.useBackgroundOpacity ^js bottom-sheet-js translate-y bg-height window-height)
                ...
                on-collapsed (fn []
                               ...
                               (reanimated/set-shared-value pan-y 0))
                bottom-sheet-gesture (get-bottom-sheet-gesture pan-y translate-y bg-height bg-height-expanded
                                                               window-height keyboard-shown disable-drag? expandable?
                                                               show-bottom-sheet? expanded? close-bottom-sheet)]
             ...
             [gesture/gesture-detector {:gesture bottom-sheet-gesture}
              [reanimated/view {:style (reanimated/apply-animations-to-style
               ...
               [rn/view {:style (styles/background)}
                [rn/keyboard-avoiding-view {:behaviour (if platform/ios? :padding :height)
                                            :style {:flex 1}}
